...
class SqliteService {
  ...
  Future _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < 2) {
      // El usuario estaba en v1, solo necesita la nueva tabla 'favoritos'
      // No tocamos 'bebidas' ni 'categorias' para no perder sus datos.
      await db.execute('''
      CREATE TABLE favoritos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id TEXT NOT NULL,
  ...
  Future<int> removeFavorito(String userId, int bebidaId) async {
    ...
    return await db.delete(
      'favoritos',
      where: 'user_id = ? AND bebida_id = ?',
      whereArgs: [userId, bebidaId],
    );
  }
  ...
  Future<List<Bebida>> getFavoritosPorUsuario(String userId) async {
    ...
    final result = await db.rawQuery('''
      ...
      FROM bebidas b
      JOIN categorias c ON b.categoria_id = c.id
      JOIN favoritos f ON b.id = f.bebida_id
      WHERE f.user_id = ?
    ''', [userId]);
